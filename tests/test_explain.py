...
def test_explain_skips_files_matching_ignore_patterns(tmp_path: Path, fake_home: Path, runner: CliRunner):
    rules = f"""
source: {tmp_path.as_posix()}
default_target: {tmp_path.as_posix()}/others
ignore:
...
""".lstrip()
    ...
    result = runner.invoke(app, ["explain", str(tmp_file), "-p", "home", "--verbose"])
    assert result.exit_code == 0, (result.stdout or "") + (result.stderr or "")

    out = (result.stdout or "") + (result.stderr or "")
    assert "notes.tmp [SKIP]" in out
    assert "ignored_by_pattern" in out
